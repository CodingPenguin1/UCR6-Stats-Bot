...
from utils import get_team_id_from_roster, get_team_ids_from_round_dict
...
def generate_players_and_teams_tables(data_dir: Path = DATA_DIR):
    ...
    def update_players_with_all_names(player_info_df: pd.DataFrame, data_dir: Path):
        ...
        for i, row in player_info_df.iterrows():
            ...
            for match_json_path in sorted(Path(data_dir / "2_decoded_replays").glob("*.json"), reverse=True):
                with open(match_json_path) as f:
                    match_json = json.load(f)
                for player_dict in match_json["rounds"][0]["players"]:
                    if player_dict["profileID"] == player_id and player_dict["username"] not in player_names:
...
def generate_rounds_table(data_dir: Path = DATA_DIR):
    def get_round_info(match_json_path: Path):
        ...
        for i, round in enumerate(match_json["rounds"]):
            ...
            team_0_player_ids = [player["profileID"] for player in round["players"] if player["teamIndex"] == 0]
            team_1_player_ids = [player["profileID"] for player in round["players"] if player["teamIndex"] == 1]
...
def decode_matches(data_dir: Path = DATA_DIR, decoder_path: Path = DECODER_PATH):
    ...
    futures = []
    thread_count = min(len(match_folders), round(_memory_available() / 6))  # overestimate of 6 GB per thread
