...
def generate_objective_table(data_dir: Path = DATA_DIR):
    def get_objective_plays(match_json_path: Path):
        def get_operator_from_player_id(player_dicts: list[dict], player_id: str):
            for player in player_dicts:
                if player["profileID"] == player_id:
                    return player["operator"]["name"]
        ...
        for i, round in enumerate(match_json["rounds"]):
            ...
            for event in round["matchFeedback"]:
                if event["type"]["name"] not in {"DefuserPlantComplete", "DefuserDisableComplete"}:
                    continue

                player_id = get_player_id_from_name(round, event["username"])
                ...
                obj_info = {
                    ...
                    "round_num": round_num,
                    "player_id": player_id,
                    "type": event["type"]["name"].removeprefix("Defuser").removesuffix("Complete"),
                    "time": event["time"],
                    "time_seconds": event["timeInSeconds"],
                    "map": round["map"]["name"],
                    "opeartor": get_operator_from_player_id(round["players"], player_id),
                    "side": "Attack"
                    if event["type"]["name"] == "DefuserPlantComplete"
                    else "Defense",
                    "spawn": get_spawn_location_from_player_id(round["players"], player_id),
                }

                round_obj_plays.append(obj_info)

            obj_plays.extend(round_obj_plays)  # Append all processed objective plays for this round

        return pd.DataFrame(obj_plays)

    obj_data_df = pd.DataFrame()
    for match_json_path in Path(data_dir / "2_decoded_replays").rglob("*.json"):
        obj_data_df = pd.concat(
            (obj_data_df, get_objective_plays(match_json_path)), ignore_index=True
        )
    return obj_data_df.sort_values(
        by=["match_id", "round_num", "time_seconds"],
        ascending=[True, True, False],
    )
...
def decode_matches(data_dir: Path = DATA_DIR, decoder_path: Path = DECODER_PATH):
    ...
    obj_data_df = generate_objective_table(data_dir)
    obj_data_df.to_csv(data_dir / "3_tables/obj_data.csv", index=False)
